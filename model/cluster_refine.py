...
# tips: change from batch to stack
    def forward(self, batch_dict):
        ...
        if new_pts is None:
            batch_dict['keypoint'] = offset_pts.new_zeros((0, 4))
            batch_dict['keypoint_features'] = offset_pts.new_zeros((0, self.num_output_feature))
            batch_dict['refined_keypoint'] = offset_pts.new_zeros((0, 3))
            return batch_dict
        ...
        xyz_batch_cnt = offset_pts.new_ones(offset_pts.shape[0], dtype=torch.int32) * N
        new_xyz_batch_cnt = new_xyz_batch_cnt[pos_mask]
        point_fea = batch_dict['point_features']
        if point_fea.dim() == 2:
            point_fea = point_fea.view(B, N, -1)
    ...
    def loss(self, loss_dict, disp_dict,batch_dict=None):
        ...
        if 'keypoint_offset_pred' not in self.train_dict:
            zero = next(self.parameters()).sum() * 0.0
            loss_dict.update({
                'refine_offset_loss': 0.0,
                'refine_loss': 0.0
            })
